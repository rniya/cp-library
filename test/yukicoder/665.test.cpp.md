...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/665.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/665\"\
    \n\n#line 2 \"polynomial/lagrange_interpolation.hpp\"\n#include <vector>\n\ntemplate\
    \ <typename M> M lagrange_interpolation(std::vector<M> x, std::vector<M> y, M\
    \ T) {\n    int n = x.size() - 1;\n    M res = 0;\n    for (int i = 0; i <= n;\
    \ i++) {\n        M c = 1;\n        for (int j = 0; j <= n; j++) {\n         \
    \   if (i == j) continue;\n            (c *= T - x[j]) /= x[i] - x[j];\n     \
    \   }\n        res += c * y[i];\n    }\n    return res;\n}\n\ntemplate <typename\
    ...
    \ 1; i++) A[i] = A[i - 1] + ((mint)i).pow(k);\n    std::cout << lagrange_interpolation_arithmetic(A,\
    \ (mint)n) << '\\n';\n}\n"
