...
data:
  ...
  bundledCode: "#line 1 \"test/aoj/2871.test.cpp\"\n#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/2871\"\
    ...
    \ int, decltype(f), decltype(g), decltype(h)> seg(n, f, g, h, node(0, 0), 0);\n\
    \    for (int i = 0; i < n; i++) seg.set(ET.idx(i), c[i] == 'G' ? node(1, 0) :\
  ...
  code: "#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/2871\"\n\n#include\
    ...
    \ return a ^ b; };\n    LazySegmentTree<node, int, decltype(f), decltype(g), decltype(h)>\
    \ seg(n, f, g, h, node(0, 0), 0);\n    for (int i = 0; i < n; i++) seg.set(ET.idx(i),\
    \ c[i] == 'G' ? node(1, 0) : node(0, 1));\n    seg.build();\n    auto F = [&](int\
    \ l, int r) { seg.update(l, r, 1); };\n\n    for (; q--;) {\n        int v;\n\
    \        cin >> v;\n        ET.exec(--v, F);\n        auto ans = seg.query(0,\
