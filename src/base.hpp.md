...
data:
  ...
  code: "#pragma once\n#include <bits/stdc++.h>\n#ifdef LOCAL\n#include <debug.hpp>\n\
    ...
    \ < x and (x = forward<U>(y), true); }\n\ntemplate <class T, class U = T> bool\
    \ chmax(T& x, U&& y) { return x < y and (x = forward<U>(y), true); }\n\ntemplate\
    \ <class T> void mkuni(vector<T>& v) {\n    sort(begin(v), end(v));\n    v.erase(unique(begin(v),\
    \ end(v)), end(v));\n}\n\ntemplate <class T> int lwb(const vector<T>& v, const\
    \ T& x) { return lower_bound(begin(v), end(v), x) - begin(v); }\n"
  dependsOn: []
